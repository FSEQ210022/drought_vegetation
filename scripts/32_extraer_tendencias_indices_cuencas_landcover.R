...
ind <- sapply(1:24,\(i) rep(i,2)) |> as.numeric()
trends <- tapp(trends,ind,prod)
names(trends) <- indices

df_scales_1 <- terra::extract(trends[[7:24]],cuencas,mean,na.rm = TRUE)
...
df_scales_2 <- terra::extract(trends[[1:6]],cuencas,mean,na.rm = TRUE)
#fun =\(x){
#   table(x) |> sort() -> x
#   as.numeric(names(x[1]))
# })
...
cuencas <- st_transform(cuencas,crs(trends))
