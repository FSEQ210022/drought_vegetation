...
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[boxrule=0pt, sharp corners, interior hidden, enhanced, borderline west={3pt}{0pt}{shadecolor}, breakable, frame hidden]}{\end{tcolorbox}}\fi
...
chilean matorral (shrubland and savanna.\citep{Fuentes2020}), followed
...
accuracy and F1 score as metrics of performance.
...
macrozones with major LULCC were ``Sur'' and ``Centro'' with \% and \%
of surface evidenced change, respectively. The ``Norte Chico'' shows an
increase in barrend land of 111 \(km^2 year^{-1}\) and a reduction in
the class savanna -70 \(km^2 year^{-1}\). In the ``Centro'' and ``Sur''
there are changes in the chilean matorral, with an important reduction
in savanna (-136 to -318 \(km^2 yr^{-1}\)), and an increase in shrubland
and grassland. Showing a change for more dense vegetation types. It
appears to be a change from the surface occupied by cropland from the
``Centro'' to the ``Sur.'' Also, there is a high increase in forest (397
\(Km^2 yr^{-1}\)) in the ``Sur,'' replacing the savanna lost.
...
Regarding vegetation productivity aggregated through the macrozones,
there is a negative trend in four macrozones; only ``Norte Grande''
shows a positive trend of 0.07 per decade (z-score). zcNDVI reached a
trend of -0.07 per decade in the ``Centro'' and ``Norte Chico,''
followed by ``Sur'' with -0.05 and ``Austral'' with -0.03
(Figure~\ref{fig-zcNDVI_var}). This could be associated either with a
reduction in vegetation surface, a decrease in biomass, or browning
\citep{Miranda2023}. It reached its lowest values since the year 2019,
reaching an extreme condition in early 2020 and 2022 in the ``Centro.''
If we observe the spatial variation in Figure~\ref{fig-zcNDVI_var} a,
despite the fact that ``Norte Grande'' has a positive trend, it can be
noticed that the south part has a cluster with a negative trend. The
negative trend is most noticeable in ``Norte Chico'' and the northern
part of ``Centro,'' but from there to the south, there is an extensive
surface of change in landcover class where the trend was not calculated.
